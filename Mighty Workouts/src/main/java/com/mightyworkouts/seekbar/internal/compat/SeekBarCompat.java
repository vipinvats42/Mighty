...
package com.mightyworkouts.seekbar.internal.compat;
...
import com.mightyworkouts.seekbar.internal.drawable.AlmostRippleDrawable;
...
public class SeekBarCompat {
     ...
     * @return
     */
    public static Drawable getRipple(Context context ,ColorStateList colorStateList) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return SeekBarCompatDontCrash.getRipple(colorStateList);
        } else {
            return new AlmostRippleDrawable(context ,colorStateList);
        }
    }
